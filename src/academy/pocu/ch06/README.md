...
#### 레드-블랙 트리 삭제 전략 - case1
- 상황 : N이 새로운 루트
  - 어떻게 재귀적으로 이리로 왔던 의미하는 바는 하나
  - 루트는 모든 하위 트리에 동일한 영향을 미침
- 전략 : 그냥 종료

#### 레드-블랙 트리 삭제 전략 - case2
- 상황 : 형제 자매인 S가 레드(P는 당연히 블랙)
- 전략
  - P, S의 색상을 바꿈
  - P를 회전하여 S가 N의 조부모가 되게 함
- N의 하위 트리에 여전히 블랙이 부족
  - case3, 4, 5, 6으로 진행

#### 레드-블랙 트리 삭제 전략 - case3
- 상황
  - P, S, Sl(S의 자식), Sr(S의 자식)이 모두 블랙
- 전략
  - S를 레드로
  - 그리고 case1로 다시 시작

#### 레드-블랙 트리 삭제 전략 - case4
- 상황
  - P가 레드(당연히 S는 블랙)
  - Sl, Sr이 블랙
- 전략
...
#### 레드-블랙 트리 삭제 전략 - case5
- 상황
  - S와 Sr이 블랙, Sl은 레드(N은 부모의 왼쪽 자식)
- 전략
  - S를 오른쪽으로 회전
...
### 레드-블랙 트리 삭제의 시간 복잡도
- BST 방식으로 노드를 제거 : O(log n)
- 망가진 레드-블랙 트리의 특성 고침 : O(log n)
  - 색상 바꾸기 : O(1)
  - 트리 회전 : O(1)
