...
- 여태까지 본 문제들은 단일 출발지 최단 경로
  - SSSP, Single-Source Shortest Path
  - 한 노드에서 시작해서 다른 모든 노드로 향하는 최단 거리를 찾음
  - 다익스트라와 A*가 이런 문제들을 해결
- 모든 노드 쌍에 대해 최단 경로를 찾는 문제들도 있음
  - APSP, All-Pairs Shortest Path
- SSSP 알고리즘을 모든 노드에서 한 번씩 시작해도 해법은 나옴
  - 하지만 시간 복잡도가 SSSP 알고리즘 시간 복잡도의 N배
  - 더 나은 APSP 전용 알고리즘이 있음
  
### 개념
- 모든 쌍의 최단 거리를 찾는 알고리즘 중 하나
- 다익스트라와 달리 인접행렬(2D)을 사용
- 다익스트라와 마찬가지로 동적 계획법 알고리즘
- 기본 원리
  - u -> v가 최단 경로인 경우
  - 중간에 거쳐가는 m노드에 대해
  - u -> m은 최단 경로
...
- sp(i, j, k)를 다음과 같이 정의
  - i -> j의 최단 경로
  - 중간 1~k 노드를 거쳐도 됨 (= K+1 ~ N 노드는 거치지 않음)
